...
function startTime(element, length) {
  ...
  //! Compute the number of questions that have been answered
  ...
  interval = setInterval(() => {
    if (timer.classList.contains("toggle"))
      setTimeout(() => {
        timer.classList.remove("toggle");
      }, 500);

    demo.style.display = "none";
    let timer_word = `Timer : 0${minutes}: `;
    //! check if seconds > 10 add seconds as is
    if (seconds >= 10) {
